...
def _run_npm_install_deterministic(
    ...
    return _run_npm_install_deterministic_with_prefix(
        [npm], cwd, extra_args=extra_args, capture_output=capture_output, env=env)


def _run_npm_install_deterministic_with_prefix(
    npm_prefix: list[str], cwd: Path, *, extra_args: tuple[str, ...] = (),
    capture_output: bool = True, env: dict[str, str] | None = None,
) -> subprocess.CompletedProcess:
    """Deterministic npm install through an executable/wrapper prefix.

    A prefix lets immutable Fedora hosts run native-addon builds through
    ``toolbox run ... npm`` without weakening the normal npm engine-repair
    path used by direct host installs.
    """
    # CI=1 no-ops unicode-animations' postinstall that animates to /dev/tty.
    run_env = _npm_lifecycle_env(env)

    def _attempt(prefix: list[str]) -> subprocess.CompletedProcess:
        def _run(args: list[str]) -> subprocess.CompletedProcess:
            return _run_npm_watching_for_engine_failure(
                [*prefix, *args, "--include=dev", *extra_args], cwd=cwd,
                env=run_env, capture_output=capture_output,
            )
    ...
    result = _attempt(npm_prefix)
    if result.returncode == 0:
        return result

    # Wrapped npm runs inside a separately managed environment; a host npm
    # engine repair cannot replace the executable inside that wrapper.
    if len(npm_prefix) != 1:
        return result

    from hermes_cli.npm_engine import maybe_repair_npm_engine
    repaired_npm = maybe_repair_npm_engine(
        npm_prefix[0], f"{result.stdout or ''}\n{result.stderr or ''}")
    if not repaired_npm:
        return result
