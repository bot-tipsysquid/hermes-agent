...
def _is_fedora_silverblue_host() -> bool:
    """Return True for Fedora Silverblue/ostree hosts."""
    if sys.platform != "linux":
        return False
    try:
...
def _desktop_auto_ozone_platform_hint() -> Optional[str]:
    """Scope the X11 compatibility fallback to affected Silverblue GNOME ARM sessions."""
    if not _is_fedora_silverblue_host():
        return None
...
def _desktop_launch_env(args: argparse.Namespace) -> tuple[dict, list[str]]:
    ...
    if config_ozone_hint != "auto" and "ELECTRON_OZONE_PLATFORM_HINT" not in os.environ:
        ...
        config_ozone_hint == "auto"
        and "ELECTRON_OZONE_PLATFORM_HINT" not in os.environ
        and "HERMES_DESKTOP_OZONE_PLATFORM" not in os.environ
        and (auto_ozone_hint := _desktop_auto_ozone_platform_hint()) is not None
    ):
        env["ELECTRON_OZONE_PLATFORM_HINT"] = auto_ozone_hint
