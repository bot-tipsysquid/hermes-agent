...
def _run_npm_install_deterministic_with_prefix(
    ...
    *,
    ...
    def _attempt(prefix: list[str]) -> subprocess.CompletedProcess:
        ...
        if lockfile.exists():
            ci_result = _run([*prefix, "ci", "--include=dev", *extra_args])
            if ci_result.returncode == 0:
                return ci_result
            # Fall through to `npm install` — lockfile may be out of sync on a
            # WIP fork/branch, or `npm ci` may not be available on very old npm.
        return _run([*prefix, "install", "--no-save", "--include=dev", *extra_args])
    ...
    # Prefix wrappers such as ``toolbox run --container NAME npm`` cannot be
    # repaired by the host-side managed npm installer. Return their real
    # failure and let the caller surface it instead of mutating host tooling.
    if len(npm_prefix) != 1:
        return result
    npm = npm_prefix[0]
    combined = f"{result.stdout or ''}\n{result.stderr or ''}"
