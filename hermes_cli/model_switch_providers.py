...
def _lap_bare_custom_row(b: _PickerBuild, custom_providers: list | None) -> None:
    ...
    try:
        ...
        if isinstance(model_cfg, dict):
            configured_url = _norm_url(model_cfg.get("base_url"))
            if configured_url and configured_url == _norm_url(api_url):
    ...
    try:
        discovered, native_catalog_empty = _discover_endpoint_models(
            api_key, api_url, "custom", False, headers=None, api_mode=None,
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    Keep the active provider first: gateway pickers render this list directly,
    and prepending MoA must not displace the provider the session is using."""
    try:
