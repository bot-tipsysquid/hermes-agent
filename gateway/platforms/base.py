...
def _extensionless_media_matches(masked: str):
    ...
    known_spans = [match.span() for match in MEDIA_TAG_CLEANUP_RE.finditer(masked)]
    for match in MEDIA_EXTENSIONLESS_TAG_RE.finditer(masked):
        # A known-extension tag containing spaces can have a shorter, valid
        # extensionless prefix on disk. The known tag owns its full span; do
        # not emit that overlapping prefix as a second attachment.
        if any(
