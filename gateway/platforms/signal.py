...
def _stage_outbound_attachments_sync(
    ...
    try:
        ...
        raise _AttachmentStagingError(str(exc)) from exc
    try:
        staged: List[str] = []
...
async def _cleanup_staged_attachments(stage_root: Path) -> bool:
    """Remove a staged tree off-loop; report whether cancellation intervened."""
...
async def _stage_outbound_attachments(
    ...
    """Stage attachments with distinct pre/post-accept cancellation semantics."""
    ...
    try:
        ...
        if staged.cancellation_requested:
            if not staged.committed:
                raise asyncio.CancelledError()
            _clear_current_task_cancellation()
...
class SignalAdapter(BasePlatformAdapter):
    ...
    async def send_multiple_images(self, chat_id: str, images: List[Tuple[str, str]],
        ...
        for idx, att_batch in enumerate(att_batches, start=1):
            ...
            accepted, cancelled_after_accept = await self._send_attachment_batch(
                scheduler, dict(base_params, attachments=att_batch), n,
                f"{idx}/{n_batches}")
            if accepted:
                delivered = True
            if cancelled_after_accept:
                break
        return SendResult(
            success=delivered,
    ...
    async def _send_attachment_batch(
        ...
        Returns ``(accepted, cancelled_after_accept)`` so a caller can stop remaining batches without
        surfacing a retryable cancellation after Signal already accepted this one."""
        send_timeout, max_attempts = _signal_send_timeout(n), SIGNAL_RATE_LIMIT_MAX_ATTEMPTS
        source_attachments = list(params.get("attachments", []))
        for attempt in range(1, max_attempts + 1):
            await scheduler.acquire(n)
            t0 = time.monotonic()
            try:
                success, err_msg, result = False, None, None
                async with _stage_outbound_attachments(source_attachments) as staged:
                    staged_params = dict(params, attachments=staged.attachments)
                    result = await self._rpc(
                        "send", staged_params, raise_on_rate_limit=True,
                    ...
                    if success:
                        ...
                        report_task = asyncio.create_task(
                            scheduler.report_rpc_duration(duration, n))
                        try:
                            if await _finish_post_commit_task(report_task):
                                staged.cancellation_requested = True
                        except Exception as exc:
                            # The message crossed Signal's acceptance boundary. Bookkeeping must never
                            # make the call look retryable and duplicate an attachment batch.
                            logger.warning(
                                "Signal batch %s: post-acceptance bookkeeping failed: %s",
                                label, exc)
                cancelled_after_accept = success and staged.cancellation_requested
            except _AttachmentStagingError as exc:
                logger.error(
                    "Signal: failed to stage batch %s (%d attachments): %s",
                    label, n, exc)
                return False, False
            ...
            if success:
                ...
                return True, cancelled_after_accept
    ...
    async def _send_file(self, chat_id: str, file_path: str, caption: Optional[str], fail_error: str) -> SendResult:
        ...
        params = await self._with_target(
            {"account": self.account, "message": caption or ""}, chat_id)
        try:
            ...
            return SendResult(success=False, error=f"Failed to stage attachment: {exc}")
