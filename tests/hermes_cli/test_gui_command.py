...
def _isolate_xdg_data_home(tmp_path, monkeypatch):
    ...
    # Launcher tests exercise build/dispatch behavior, not host-specific Ozone
    # selection. Keep the ambient Wayland session from changing expected argv;
    # dedicated tests below cover the automatic X11 compatibility flag.
    monkeypatch.setenv("HERMES_DESKTOP_OZONE_PLATFORM", "auto")
...
def test_gui_install_env_prepends_managed_node_on_bare_path(tmp_path, monkeypatch):
    ...
    with patch("hermes_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("hermes_cli.main._desktop_toolbox_build_container", return_value=None), \
         patch("hermes_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("hermes_cli.main._desktop_build_needed", return_value=True), \
         patch("hermes_cli.main._write_desktop_build_stamp"), \
         patch("hermes_cli.main._desktop_macos_relaunchable_fixup"), \
...
def test_desktop_default_ozone_platform_flags_honor_explicit_env(monkeypatch):
    ...
    assert cli_main._desktop_default_ozone_platform_flags(
        {
            "HERMES_DESKTOP_OZONE_PLATFORM": "wayland",
...
def test_desktop_default_ozone_platform_flags_allow_opt_out_and_user_flag(monkeypatch):
    monkeypatch.setattr(cli_main.sys, "platform", "linux")
    env = {
        "HERMES_DESKTOP_OZONE_PLATFORM": "auto",
        "XDG_SESSION_TYPE": "wayland",
        "DISPLAY": ":0",
    }
...
def test_desktop_default_ozone_platform_flags_only_on_linux_wayland(monkeypatch):
...
def test_desktop_dependency_install_uses_toolbox_when_host_lacks_build_tools(
    tmp_path, capsys
):
    root = tmp_path / "hermes-agent"
    root.mkdir()
    ...
    with (
        patch(
            "hermes_cli.main._desktop_toolbox_build_container",
            return_value="hermes-arm-build",
        ),
        patch(
            "hermes_cli.main._run_npm_install_deterministic_in_toolbox",
            return_value=ok,
        ) as toolbox_install,
        patch("hermes_cli.main._run_npm_install_deterministic") as host_install,
    ):
        result = cli_main._run_desktop_dependency_install(
            "/usr/bin/npm", root, capture_output=False, env={"EXAMPLE": "1"}
        )

    assert result is ok
    host_install.assert_not_called()
    toolbox_install.assert_called_once_with(
        "hermes-arm-build",
        root,
...
def test_npm_install_in_toolbox_wraps_deterministic_prefix(tmp_path, monkeypatch):
    root = tmp_path / "hermes-agent"
    root.mkdir()
    ok = subprocess.CompletedProcess([], 0)
    monkeypatch.setattr(
    ...
    with patch(
        "hermes_cli.main._run_npm_install_deterministic_with_prefix",
        return_value=ok,
    ) as prefixed_install:
    ...
    assert result is ok
    prefixed_install.assert_called_once_with(
...
def test_desktop_toolbox_build_container_prefers_hermes_arm_build(monkeypatch):
    list_result = subprocess.CompletedProcess(
        ...
        stdout=(
            ...
            "b6ec71e08383  hermes-arm-build  9 days ago  running  "
            "registry.fedoraproject.org/fedora-toolbox:44\n"
        ),
    )
    probe_ok = subprocess.CompletedProcess(["toolbox", "run"], 0)
    monkeypatch.setattr(
        cli_main, "_fedora_silverblue_without_host_make", lambda: True
    )
    monkeypatch.setattr(
        cli_main.shutil,
        "which",
    ...
    def fake_run(command, *args, **kwargs):
        ...
        if command[:3] == ["/usr/bin/toolbox", "list", "--containers"]:
            return list_result
        return probe_ok
