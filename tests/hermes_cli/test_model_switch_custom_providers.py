...
def test_active_bare_custom_probe_uses_configured_model_api_key(monkeypatch):
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr(providers_mod, "HERMES_OVERLAYS", {})
    monkeypatch.setattr(
    ...
    def fake_cached_fetch(api_key, base_url, **kwargs):
        ...
        return ["venice-model-a", "venice-model-b"]

    monkeypatch.setattr(
        "hermes_cli.models.cached_fetch_api_models", fake_cached_fetch
    )

    providers = list_authenticated_providers(
        current_provider="custom",
        current_base_url="https://example.invalid/api/v1",
        current_model="venice-model-a",
        user_providers={},
        custom_providers=[],
        probe_custom_providers=False,
        probe_current_custom_provider=True,
    ...
    assert calls == [
        (
            ...
            {"timeout": 5.0, "headers": None, "api_mode": None},
        )
    ]
    assert bare_custom["models"] == ["venice-model-a", "venice-model-b"]


def test_active_bare_custom_probe_does_not_leak_key_to_session_override(monkeypatch):
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr(providers_mod, "HERMES_OVERLAYS", {})
    monkeypatch.setattr(
        "hermes_cli.config.load_config",
        lambda: {
            "model": {
                "api_key": "configured-endpoint-secret",
    ...
    list_authenticated_providers(
        current_provider="custom",
        current_base_url="https://different-session-endpoint.example/v1",
    ...
    assert calls == [("", "https://different-session-endpoint.example/v1")]
