...
import { resolveOzonePlatformSwitch } from './ozone-platform'
...
// Fedora/GNOME Wayland can keep Electron windows alive but unmapped on some
// ARM hosts. Respect an explicit CLI/config choice first, then prefer the
// available XWayland backend for that known-bad environment. This must run
// before app `ready` so Chromium receives the switch during startup.
const OZONE_PLATFORM_SWITCH = resolveOzonePlatformSwitch()

if (OZONE_PLATFORM_SWITCH) {
  app.commandLine.appendSwitch('ozone-platform', OZONE_PLATFORM_SWITCH)
  console.log(`[hermes] forcing Electron ozone platform: ${OZONE_PLATFORM_SWITCH}`)
}
