...
function spawnSecondaryWindow({
  ...
  installDeferredShow(win, { label: 'session', rememberLog })
...
function createWindow() {
  ...
  installDeferredShow(mainWindow, {
    label: 'main',
    rememberLog,
    onShown: () => {
      ...
      if (IS_WINDOWS) {
        try {
          ...
          rememberLog(`[sandbox] marker update after window show failed: ${error?.message || error}`)
        }
      }
    }
  })
